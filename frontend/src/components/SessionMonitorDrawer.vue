        ...
        <div class="mid">
          ...
          <div class="sess-bar">
            <i
              :style="{
                width:
                  Math.round((session.event_count / maxEventCount) * 100) + '%',
              }"
            />
          </div>
          ...
          <ul
            ...
            class="usage-skill-list"
            data-test="session-events-list"
          >
            <li
              ...
              :class="{
                'usage-skill-item--active': activePaths.has(event.skill_path),
              }"
...
const visibleSessions = computed(() =>
  props.usageSessions.filter((s) => s.event_count > 0),
);
const maxEventCount = computed(() =>
  Math.max(1, ...visibleSessions.value.map((s) => s.event_count)),
);
...
watch(
  () => props.autoExpandSessionId,
  async (id) => {
    if (!id) return;
    expandedSessions.value = new Set();
    if (!sessionEventsCache.value[id]) {
      sessionEventsCache.value = {
        ...sessionEventsCache.value,
        [id]: await getSessionEvents(id),
      };
    }
...
async function toggleSessionEvents(sessionId) {
  if (expandedSessions.value.has(sessionId)) {
    expandedSessions.value = new Set(
      [...expandedSessions.value].filter((id) => id !== sessionId),
    );
  } else {
